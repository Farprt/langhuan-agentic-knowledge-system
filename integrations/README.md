...
The repository intentionally does not copy Honcho or Langfuse source, install a commercial probe, start containers, or enable cloud upload automatically. Only an explicit exporter command with `--send` can upload events. Follow upstream installation documentation, pin a reviewed version, and keep credentials outside Git.
